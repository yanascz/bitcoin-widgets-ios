        ...
        VStack(alignment: .trailing) {
            if family == .accessoryInline {
                ...
                Text(verbatim: String(mempoolStatus.blockHeight))
                    .font(.system(size: 37))
                VStack {
                    Text(mempoolFee: mempoolStatus.economyFee)
                    + Text(verbatim: Self.feeSeparator).foregroundColor(.secondary)
                    + Text(mempoolFee: mempoolStatus.hourFee)
                    + Text(verbatim: Self.feeSeparator).foregroundColor(.secondary)
                    + Text(mempoolFee: mempoolStatus.halfHourFee)
                    + Text(verbatim: Self.feeSeparator).foregroundColor(.secondary)
                    + Text(mempoolFee: mempoolStatus.fastestFee)
    ...
    func systemView(for family: WidgetFamily) -> some View {
        VStack(alignment: family == .systemSmall ? .trailing : .center) {
            Text(verbatim: String(mempoolStatus.blockHeight))
                ...
                .padding(.bottom, 1)
            VStack(alignment: .trailing) {
                Text(mempoolFee: mempoolStatus.economyFee).foregroundColor(.white)
                + Text(verbatim: Self.feeSeparator).foregroundColor(.gray)
                + Text(mempoolFee: mempoolStatus.hourFee).foregroundColor(.white)
                + Text(verbatim: Self.feeSeparator).foregroundColor(.gray)
                + Text(mempoolFee: mempoolStatus.halfHourFee).foregroundColor(.white)
                + Text(verbatim: Self.feeSeparator).foregroundColor(.gray)
                + Text(mempoolFee: mempoolStatus.fastestFee).foregroundColor(.white)
                Text("MempoolStatusView.minimumFee").foregroundColor(.gray)
                + Text(verbatim: " ")
                + Text(mempoolFee: mempoolStatus.minimumFee).foregroundColor(.white)
